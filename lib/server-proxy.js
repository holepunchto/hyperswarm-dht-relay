...
class ServerProxy extends EventEmitter {
  constructor (dht, socket, protocol, keyPair) {
    ...
    this._signing = new SigningProxy(this._protocol, keyPair.publicKey)

    const custodial = keyPair.secretKey !== null

    this._server = dht.createServer({
      createHandshake: custodial ? null : this._noise.createHandshake(),
      createSecretStream: custodial ? null : this._noise.createSecretStream()
    })
    ...
    this._server
      ...
      .listen({
        ...
        signAnnounce: custodial ? null : this._signing.signAnnounce,
        signUnannounce: custodial ? null : this._signing.signUnannounce
      })
...
async function onConnection (socket) {
  const id = socket._socketId || crypto.randomId()
