  ...
  constructor (dht, socket, protocol, id, keyPair) {
    ...
    this._server = dht.createServer({
      createHandshake: custodial
        ? null
        : noise.createHandshake(this._protocol, crypto.randomId(), this.id),
    ...
    const signing = new SigningProxy(this._protocol, crypto.hash(keyPair.publicKey), keyPair.publicKey)
