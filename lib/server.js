...
async function onConnection (message) {
  ...
  await this._protocol.accept({ alias, remoteAlias })

  if (this._custodial) return

  const handshake = this._node._handshakes.get(remoteAlias)
...
function onConnected (message) {
  ...
  if (socket) this.emit('connection', socket.noiseStream)
}
