...
const heartbeatFrequency = 30 * 1e3
...
class Protocol extends EventEmitter {
  ...
  async _onData (buffer) {
    ...
    switch (type) {
      ...
      case 10: return this.emit('data', messages.data.decode(state))
      case 11: return this.emit('lookup', messages.lookup.decode(state))
      case 12: return this.emit('announce', messages.announce.decode(state))
      case 13: return this.emit('unannounce', messages.unannounce.decode(state))
    }
  }
  ...
  async data (message) {
    await this.send(10, messages.data, message)
  }

  async lookup (message) {
    await this.send(11, messages.lookup, message)
  }

  async announce (message) {
    await this.send(12, messages.announce, message)
  }

  async unannounce (message) {
    await this.send(13, messages.unannounce, message)
  }
}
