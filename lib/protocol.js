...
class Protocol extends EventEmitter {
  ...
  async _onMessage (buffer) {
    ...
    switch (type) {
      ...
      case 5: return this.emit('connect', messages.connect.decode(state))
      case 6: return this.emit('connected', messages.connected.decode(state))
      case 7: return this.emit('destroy', messages.destroy.decode(state))
      case 8: return this.emit('join', messages.join.decode(state))
      case 9: return this.emit('leave', messages.leave.decode(state))
      case 10: return this.emit('connection', messages.connection.decode(state))
      case 11: return this.emit('data', messages.data.decode(state))
      case 12: return this.emit('flush', messages.flush.decode(state))
      case 13: return this.emit('flushed', messages.flushed.decode(state))
      case 14: return this.emit('refresh', messages.refresh.decode(state))
    }
  }
  ...
  async connect (message) {
    await this.send(5, messages.connect, message)
  }

  async connected (message) {
    await this.send(6, messages.connected, message)
  }

  async destroy (message) {
    await this.send(7, messages.destroy, message)
  }

  async join (message) {
    await this.send(8, messages.join, message)
  }

  async leave (message) {
    await this.send(9, messages.leave, message)
  }

  async connection (message) {
    await this.send(10, messages.connection, message)
  }
  ...
  async flush (message) {
    await this.send(12, messages.flush, message)
  }

  async flushed (message) {
    await this.send(13, messages.flushed, message)
  }

  async refresh (message) {
    await this.send(14, messages.refresh, message)
  }
