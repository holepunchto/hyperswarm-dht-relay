...
class Protocol extends EventEmitter {
  ...
  async noiseRequest (message) {
    await this.send(m.noiseRequest, message)
  }

  async noiseResponse (message) {
    await this.send(m.noiseResponse, message)
  }
}
...
function encodingFor (type) {
  switch (type) {
    ...
    case m.error.type: return m.error
    ...
    case m.noiseRequest.type: return m.noiseRequest
    case m.noiseResponse.type: return m.noiseResponse
    default: return null
  }
}
