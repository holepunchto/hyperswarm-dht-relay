...
async function onAccept (message) {
  for (const server of this._servers.values()) {
    ...
    if (socket) {
      server._connecting.delete(message.remoteAlias)

      server._connections.set(message.alias, socket)
...
function createHandshake (id, remoteSocketAlias, keyPair, remotePublicKey) {
