...
function onConnect (message) {
  ...
  const socket = this._dht.connect(message.remotePublicKey, {
    ...
    createHandshake: custodial
      ? null
      : createHandshake.bind(this, alias, remoteAlias),
...
function onNoiseReply (message) {
  ...
  if (socket) socket._handshake._onNoiseReply(message)
}

function createHandshake (alias, remoteAlias, keyPair, remotePublicKey) {
  const isInitiator = !!remotePublicKey

  const handshake = new HandshakeProxy(this._protocol, alias, remoteAlias, isInitiator, keyPair, remotePublicKey)

  const socket = this._connections.get(remoteAlias)
...
function createSecretStream (alias, remoteAlias, isInitiator, rawStream, options) {
  const socket = new SocketProxy(this._protocol, alias, remoteAlias, isInitiator, rawStream, options)
