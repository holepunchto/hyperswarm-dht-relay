...
function onConnect (message) {
  ...
  const socket = this._dht.connect(message.remotePublicKey, {
    ...
    createHandshake: custodial ? null : noise.createHandshake(this._protocol, id),
    createSecretStream: custodial ? null : noise.createSecretStream(this._protocol, id)
  })
...
function onListen (message) {
  const server = new ServerProxy(this._dht, this._socket, this._protocol, message.server, message)
...
function onAnnounce (message) {
  const custodial = false

  const signing = new SigningProxy(this._protocol, message.topic, message.publicKey)
