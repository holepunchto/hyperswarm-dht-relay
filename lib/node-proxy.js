...
function onConnect (message) {
  ...
  const socket = this._dht.connect(message.remotePublicKey, {
    ...
    createHandshake: custodial
      ? null
      : this._noise.createHandshake.bind(this._noise, id),
    createSecretStream: custodial
      ? null
      : this._noise.createSecretStream.bind(this._noise, id)
  })
...
function onListen (message) {
  const server = new ServerProxy(this, this._socket, this._protocol, message)
