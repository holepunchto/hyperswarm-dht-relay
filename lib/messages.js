const { fixed32, fixed64, buffer, raw, none, string } = require('compact-encoding')
const { ipv4Address } = require('compact-encoding-net')

const { publicKey, secretKey, id, batch, topic, peerId, token, relayAddresses } = require('./codecs')
...
const destroy = {
  preencode (state, m) {
    state.end++ // Flags
    if (m.alias) id.preencode(state, m.alias)
  ...
  decode (state) {
    ...
    return {
      alias: (flags & 1) === 0 ? null : id.decode(state),
      remoteAlias: (flags & 1) !== 0 ? null : id.decode(state),
