const { fixed, fixed32, fixed64, string, buffer, array, raw, none, bool } = require('compact-encoding')
...
const signAnnounce = message(17, 'signAnnounce', {
  preencode (state, m) {
    id.preencode(state, m.id)
    publicKey.preencode(state, m.publicKey)
    token.preencode(state, m.token)
...
const signUnannounce = message(18, 'signUnannounce', signAnnounce)

const signature = message(19, 'signature', {
  preencode (state, m) {
    id.preencode(state, m.id)
...
const noiseRequest = message(20, 'noiseRequest', {
  preencode (state, m) {
    socket.preencode(state, m.socket)
    publicKey.preencode(state, m.publicKey)
    state.end++ // Type
  ...
  encode (state, m) {
    socket.encode(state, m.socket)
    publicKey.encode(state, m.publicKey)
    state.buffer[state.start++] = m.type
  ...
  decode (state) {
    return {
      socket: socket.decode(state),
      publicKey: publicKey.decode(state),
      type: state.buffer[state.start++],
      data: buffer.decode(state)
    }
  }
})

const noiseResponse = message(21, 'noiseResponse', {
  preencode (state, m) {
    socket.preencode(state, m.socket)
    publicKey.preencode(state, m.publicKey)
    buffer.preencode(state, m.data)
    bool.preencode(state, m.complete)
  ...
  decode (state) {
    const response = {
      ...
      publicKey: publicKey.decode(state),
      data: buffer.decode(state),
      complete: bool.decode(state),
      remoteId: null,
      remotePublicKey: null,
      holepunchSecret: null
    }

    if (response.complete) {
      response.remoteId = fixed32.decode(state)
      response.remotePublicKey = publicKey.decode(state)
      response.holepunchSecret = fixed32.decode(state)
    }
