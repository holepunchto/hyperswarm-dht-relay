...
const { HandshakeSet } = require('./handshake-set')
...
class Node extends EventEmitter {
  constructor (socket, protocol, options = {}) {
    ...
    this._handshakes = new HandshakeSet()
  ...
  connect (remotePublicKey, options = {}) {
    ...
    this._handshakes.add(id, keyPair.publicKey, handshake)

    handshake.once('complete', () => {
      this._handshakes.delete(id, keyPair.publicKey)
...
function onNoiseRequest (message) {
  const handshake = this._handshakes.get(message.socket, message.publicKey)
  ...
  if (handshake) {
    ...
    if (server) {
      ...
      this._handshakes.add(message.socket, message.publicKey, handshake)
