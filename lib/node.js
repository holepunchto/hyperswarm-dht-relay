...
class Node extends EventEmitter {
  ...
  connect (remotePublicKey, options = {}) {
    ...
    const stream = new Stream(this._protocol, alias, true, {
    ...
    stream.once('close', () => this._connecting.delete(alias))
  ...
  lookup (topic) {
    const query = new Query(this._stream, this._protocol, topic, m.announcers)
...
function onOpen (message) {
  ...
  if (stream) {
    ...
    stream.once('close', () => this._connections.delete(message.alias))
