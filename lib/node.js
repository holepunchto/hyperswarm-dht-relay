...
class Node extends EventEmitter {
  ...
  connect (remotePublicKey, options = {}) {
    ...
    const socket = new Socket(this._protocol, id, true, {
      publicKey: keyPair.publicKey,
      remotePublicKey
    })

    this._connecting.set(id, socket)

    socket.once('close', () => this._connecting.delete(id))
  ...
  announce (topic, keyPair) {
    ...
    if (!this._custodial) {
      this._signings.set(query.id, new Signing(this._protocol, topic, keyPair))
    }
    ...
    query.once('close', () => {
      ...
      if (!this._custodial) {
        this._signings.delete(query.id)
      }
    })
...
async function onClose () {
  ...
  this._protocol
    ...
    .off('noiseRequest', this._onNoiseRequest)
...
function onSignAnnounce (message) {
  const signing = this._signings.get(message.signer)

  if (signing) signing._onSignAnnounce(message)
}

function onSignUnannounce (message) {
  const signing = this._signings.get(message.signer)

  if (signing) signing._onSignUnannounce(message)
}

function onNoiseRequest (message) {
  const handshake = this._handshakes.get(message.socket)

  if (handshake) handshake._onNoiseRequest(message)
  else {
    const server = this._servers.get(message.server)
