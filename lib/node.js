...
class Node extends EventEmitter {
  ...
  announce (topic, keyPair) {
    ...
    if (!this._custodial) {
      this._signings.set(keyPair.publicKey, new Signing(this._protocol, topic, keyPair))
    }
    ...
    query.once('close', () => {
      ...
      if (!this._custodial) {
        this._signings.delete(keyPair.publicKey)
      }
    })
...
function onSignAnnounce (message) {
  const signing = this._signings.get(message.publicKey)
